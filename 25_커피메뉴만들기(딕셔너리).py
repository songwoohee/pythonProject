menu_dic = {
    # 메뉴 이름 :   카테코리, 가격, 설명 순
    "Americano": ["Coffee", 2000, "기본 커피 입니다."],  # 키와 값으로 구성 되며, 값을 리스트로 만들어 여러가지 정보를 포함시킴
...
while True :
    print("메뉴를 선택 하세요 ")
    menu = input("[1] 메뉴리스트 보기 [2] 메뉴 조회 [3] 메뉴 추가 [4] 메뉴 삭제 [5] 메뉴 종료 : ") # input은 기본 문자열로 반환됨
    if menu == "1" :
        for key in menu_dic :
        ...
        add_menu = input("추가할 메뉴를 입력 하세요 : ")
        if add_menu not in menu_dic :
            category = input("카테고리 입력 : ")
            price = int(input("가격 입력 : "))
            desc = input("제품 설명 : ")
            menu_dic[add_menu] = [category, price, desc]  # 딕셔너리[키] = 값 / 새로운 값 추가
